...
import us.panks.ui.register.RegisterViewModel
...
fun RegisterPage() {
    ...
    Column(
        ...
        Column(
            ...
            SpanText("Create account", HeadlineTextStyle.toModifier().fontSize(1.75.cssRem))
            SpanText("Get started in seconds", Modifier.fontSize(0.9.cssRem).color(ColorMode.current.toSitePalette().mutedText))

            Column(Modifier.fillMaxWidth().gap(1.cssRem)) {
                Column(Modifier.gap(0.4.cssRem).fillMaxWidth()) {
                    SpanText("Name", FormLabelStyle.toModifier())
                    StyledInput(vm.name, onTextChange = { vm.name = it }, placeholder = "John Doe")
                }
                Column(Modifier.gap(0.4.cssRem).fillMaxWidth()) {
                ...
                Column(Modifier.gap(0.4.cssRem).fillMaxWidth()) {
                    SpanText("Age", FormLabelStyle.toModifier())
                    StyledInput(vm.age, onTextChange = { vm.age = it.filter { c -> c.isDigit() } }, placeholder = "30")
                }
                Column(Modifier.gap(0.4.cssRem).fillMaxWidth()) {
                ...
                Button(
                    onClick = { vm.register(scope) { ctx.router.tryRoutingTo("/users") } },
                    enabled = !vm.loading,
                    variant = PrimaryButtonVariant,
                    modifier = Modifier.fillMaxWidth(),
                ) {
                    SpanText(if (vm.loading) "Creating..." else "Create account")
                }

                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
                    SpanText("Have an account? ", Modifier.fontSize(0.9.cssRem).color(ColorMode.current.toSitePalette().mutedText))
                    Link("/login", "Login", Modifier.fontSize(0.9.cssRem).fontWeight(500))
                }
            }
        }
    }
